...
MessageBoxA:
    ...
    ; Setting up the title (Here is the flag)
    push edx                        ; null terminator
    push 'flag'						; "flag"	
    push 'the '                     ; "the "
    push ' is '                     ; " is "
    push 'Here'                     ; "Here" 
    mov edi, esp                    ; save pointer to title

    ; Setting up the flag (bbctf{Ag3ndA_3xp0s3d})
    push edx
    push '}'						; "}"
    push '0s3d'                     ; "0s3d"
    push '_3xp'                     ; "_3xp"
    ...
    push 'f{Ag'                     ; "f{Ag"
    push 'bbct'                     ; "bbct"
    mov ecx, esp                    ; save pointer to message

    push edx                        ; hWnd = NULL
    push edi                        ; pointer to title
    push ecx                        ; pointer to message
    push edx                        ; uType = NULL
